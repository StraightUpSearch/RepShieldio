...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.patch('/api/admin/tickets/:id', isAdmin, async (req: any, res) => {
    try {
      ...
      if (!ticket) ticket = await storage.getTicket(parseInt(id));
...
Date: ${new Date().toISOString()}
  ...
  // Guest checkout — email-verified, no account needed (for ticket-status page)
  app.post('/api/payments/checkout-guest', async (req: any, res) => {
    if (!isStripeConfigured()) {
      return res.status(503).json({ message: 'Payment processing is not yet active. We will contact you to arrange payment directly.' });
    }
    try {
      const { ticketId, email } = req.body;
      if (!ticketId || !email) return res.status(400).json({ message: 'ticketId and email are required' });

      const ticket = await storage.getTicket(ticketId);
      if (!ticket) return res.status(404).json({ message: 'Ticket not found' });
      if (ticket.status !== 'approved') return res.status(400).json({ message: 'This ticket is not ready for payment yet' });
      if (!ticket.amount) return res.status(400).json({ message: 'No price has been set for this ticket' });

      // Verify email matches the ticket owner
      const owner = await storage.getUser(ticket.userId);
      if (!owner || owner.email.toLowerCase() !== email.toLowerCase()) {
        return res.status(403).json({ message: 'Email does not match this ticket' });
      }
      ...
      const session = await createTicketCheckoutSession({
        ...
        description: ticket.title || 'Content removal service',
        customerEmail: email,
        successUrl: `${base}/ticket-status?payment=success`,
        cancelUrl: `${base}/ticket-status?payment=cancelled`,
      });
      ...
      switch (event.type) {
        case 'checkout.session.completed': {
          ...
          if (metadata.type === 'ticket_payment' && metadata.ticketId) {
            ...
            await storage.updateTicketStatus(ticketId, 'processing');
            ...
            try {
              await ticketLifecycle.transitionTicket({ ticketId, newStatus: 'processing' });
            } catch {}
          }
