...
export interface IStorage {
  ...
  updateTicket(id: number, fields: Record<string, any>): Promise<Ticket | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  async updateTicket(id: number, fields: Record<string, any>): Promise<Ticket | undefined> {
    try {
      const [ticket] = await db
        .update(tickets)
        .set({ ...fields, updatedAt: toDbTimestamp(new Date()) } as any)
        .where(eq(tickets.id, id))
        .returning();
      return ticket ? convertTicketFromDb(ticket) : undefined;
    } catch (error) {
...
export class MemStorage implements IStorage {
  ...
  async updateTicket(id: number, fields: Record<string, any>): Promise<Ticket | undefined> {
    const idx = this.allTickets.findIndex(t => t.id === id);
    if (idx !== -1) {
      this.allTickets[idx] = { ...this.allTickets[idx], ...fields, updatedAt: new Date() };
      return this.allTickets[idx];
    }
    return undefined;
  }
