...
import { Download, Eye, Users, FileText, Clock, CheckCircle, AlertTriangle, RefreshCw } from "lucide-react";
          ...
          <Card className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {tickets?.filter((t: Ticket) => t.status === 'processing').length || 0}
                </p>
              </div>
            </div>
          </Card>
...
function TicketManagement({ ticket, onUpdate }: { ticket: Ticket; onUpdate: (updates: any) => void }) {
  ...
  const [amount, setAmount] = useState((ticket as any).amount || '');
  ...
  const handleSave = () => {
    ...
    if (amount !== ((ticket as any).amount || '')) updates.amount = amount;
    if (assignedTo !== ticket.assignedTo) updates.assignedTo = assignedTo;
    if (notes !== ticket.notes) updates.notes = notes;
    onUpdate(updates);
  };

  const redditUrl = (ticket as any).redditUrl || ticket.requestData?.redditUrl;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg bg-gray-50 p-4">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div>
            ...
            <span className="font-medium">{ticket.requestData?.email || 'N/A'}</span>
          </div>
          <div>
            <span className="text-gray-500">Name: </span>
            <span className="font-medium">{ticket.requestData?.name || 'N/A'}</span>
          </div>
          {redditUrl && (
            <div className="col-span-2">
              <span className="text-gray-500">URL: </span>
              <a href={redditUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline break-all text-xs">
                {redditUrl}
              </a>
            </div>
          )}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <Select value={status} onValueChange={setStatus}>
              ...
              <SelectContent>
                ...
                <SelectItem value="approved">Quote sent — awaiting payment</SelectItem>
                <SelectItem value="processing">In progress</SelectItem>
                <SelectItem value="completed">Completed</SelectItem>
          ...
          <div>
            <Label htmlFor="amount" className="text-sm font-medium">
              Quote amount (USD) <span className="text-gray-400 font-normal">— triggers Pay Now</span>
            </Label>
            ...
            <p className="text-xs text-gray-400 mt-1">Set amount + status to "Quote sent" to surface the Pay Now button for the customer.</p>
          </div>
        </div>
      ...
      <div className="flex justify-end pt-2 border-t">
        <Button onClick={handleSave} className="bg-gray-950 hover:bg-gray-800 text-white">
          Save changes
        </Button>
      </div>
    </div>
